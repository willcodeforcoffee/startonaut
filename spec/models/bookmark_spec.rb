...
RSpec.describe SiteBookmark, type: :model do
  describe 'validations' do
    it "should be valid for default from FactoryBot" do
      bookmark = FactoryBot.build(:site_bookmark)
      expect(bookmark).to be_valid
    end

    it "should validate presence of url" do
      bookmark = FactoryBot.build(:site_bookmark, url: nil)
    ...
    context 'duplicate url validation' do
      it "should validate for duplicate url per user" do
        ...
        FactoryBot.create(:site_bookmark, user: user, url: "http://example.com")
        duplicate_bookmark = FactoryBot.build(:site_bookmark, user: user, url: "http://example.com")
      ...
      it "should allow the same url for different users" do
        ...
        bookmark1 = FactoryBot.create(:site_bookmark, user: user1, url: "http://example.com")
        bookmark2 = FactoryBot.build(:site_bookmark, user: user2, url: "http://example.com")
    ...
    context 'unsafe urls' do
      it "should not allow invalid URLs" do
        bookmark = FactoryBot.build(:site_bookmark, url: "invalid_url")
        expect(bookmark).not_to be_valid
        expect(bookmark.errors[:url]).to include("must be a valid HTTP or HTTPS URL")
      end

      it "should allow valid URLs" do
        bookmark = FactoryBot.build(:site_bookmark, url: "http://example.com")
        expect(bookmark).to be_valid
      end

      it "should stop javascript URLs" do
        bookmark = FactoryBot.build(:site_bookmark, url: "javascript:alert('XSS')")
        expect(bookmark).not_to be_valid
        expect(bookmark.errors[:url]).to include("must be a valid HTTP or HTTPS URL")
      end

      it "should not pass just because http is in the URL" do
        bookmark = FactoryBot.build(:site_bookmark, url: "ftp://https.example.com")
        expect(bookmark).not_to be_valid
        expect(bookmark.errors[:url]).to include("must be a valid HTTP or HTTPS URL")
      end
    end
    ...
    context 'feed_url validation' do
      it "should allow nil feed_url" do
        bookmark = FactoryBot.build(:site_bookmark, feed_url: nil)
        expect(bookmark).to be_valid
      end

      it "should allow valid HTTP feed URLs" do
        bookmark = FactoryBot.build(:site_bookmark, feed_url: "http://example.com/rss.xml")
        expect(bookmark).to be_valid
      end

      it "should allow valid HTTPS feed URLs" do
        bookmark = FactoryBot.build(:site_bookmark, feed_url: "https://example.com/feed.xml")
        expect(bookmark).to be_valid
      end

      it "should not allow invalid feed URLs" do
        bookmark = FactoryBot.build(:site_bookmark, feed_url: "invalid_feed_url")
        expect(bookmark).not_to be_valid
        expect(bookmark.errors[:feed_url]).to include("must be a valid HTTP or HTTPS URL")
      end

      it "should not allow FTP feed URLs" do
        bookmark = FactoryBot.build(:site_bookmark, feed_url: "ftp://example.com/feed.xml")
        expect(bookmark).not_to be_valid
        expect(bookmark.errors[:feed_url]).to include("must be a valid HTTP or HTTPS URL")
      end

      it "should not allow feed URLs without protocol" do
        bookmark = FactoryBot.build(:site_bookmark, feed_url: "example.com/rss.xml")
        expect(bookmark).not_to be_valid
        expect(bookmark.errors[:feed_url]).to include("must be a valid HTTP or HTTPS URL")
      end

      it "should not allow javascript URLs in feed_url" do
        bookmark = FactoryBot.build(:site_bookmark, feed_url: "javascript:alert('XSS')")
        expect(bookmark).not_to be_valid
        expect(bookmark.errors[:feed_url]).to include("must be a valid HTTP or HTTPS URL")
      end
    end
    ...
    it "should validate presence of user" do
      bookmark = FactoryBot.build(:site_bookmark, user: nil)
      expect(bookmark).not_to be_valid
